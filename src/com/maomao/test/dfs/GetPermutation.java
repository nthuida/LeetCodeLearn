...
public class GetPermutation {
    ...
    public List<String> permute(int n) {
        ...
        backtrack(num , track, result, visited);
        return result;
    }

    public void backtrack(int[] num, StringBuilder track, List<String> result, boolean[] visited) {
        // 触发结束条件
    ...
    /**
     * 通过计算剩余数字个数的阶乘数，一位一位选出第 k 个排列的数位。
     *
    ...
    private void dfs(StringBuilder path, boolean[] used, int depth, int n, int k) {
        ...
        // 计算未确定的数字的全排列的个数，第 1 次进入的时候是 n - 1
        ...
        for (int i = 1; i <= n; i++) {
            ...
            if (cur < k) {
                //当前的排列组合数小于k，说明就算当前分支排完了，也到不了第k个，剪枝
                k -= cur;
                continue;
            }
            //当前层分支满足条件的数字
